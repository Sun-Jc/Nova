//! This example executes a batch of 64-bit AND operations using Neutron.
//! It performs the AND operation by first decomposing the operands into bits and then performing the operation bit-by-bit.
//! We execute a configurable number of AND operations per step of Neutron's recursion.
...
use nova_snark::{
  ...
  neutron::{CompressedSNARK, PublicParams, RecursiveSNARK},
...
/// cargo run --release --example and_neutron
fn main() {
  println!("=========================================================");
  println!("Neutron-based 64-bit bitwise AND example");
  println!("=========================================================");
  ...
  for num_ops_per_step in [1024] {
    ...
    let (pk, vk) = CompressedSNARK::<_, _, _, S>::setup(&pp).unwrap();

    let start = Instant::now();

    let res = CompressedSNARK::<_, _, _, S>::prove(&pp, &pk, &recursive_snark);
    println!(
