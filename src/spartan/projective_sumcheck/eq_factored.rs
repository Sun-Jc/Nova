...
impl<E: Engine> EqFactoredVirtualPolynomial<E> {
  ...
  /// After all variables are bound, the reduced value `F_j(r)` of each **real
  /// (non-eq)** factor, in factor order. The eq and `U` factors are carried
  /// analytically (not in `factors`) and are not included; a caller reusing
  /// these as committed-column openings only needs the real factors.
